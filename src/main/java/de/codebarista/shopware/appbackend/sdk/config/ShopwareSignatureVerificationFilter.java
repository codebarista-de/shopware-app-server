...
public class ShopwareSignatureVerificationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        if (isValidAppSignature(request, app)) {
            setAuthentication(app.getAppName(), AppBackendSdkWebSecurityConfiguration.ROLE_SHOPWARE_APP);
            filterChain.doFilter(request, response);
            return;
        }
        ...
        if (HttpMethod.GET.matches(request.getMethod())) {
            ...
            if (info.isError()) {
                ...
                setAuthentication(info.getResult().shopId, AppBackendSdkWebSecurityConfiguration.ROLE_SHOPWARE_SHOP);
            }
            filterChain.doFilter(request, response);
            return;
        }
