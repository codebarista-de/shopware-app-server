...
import de.codebarista.shopware.appbackend.sdk.api.ApiConstants;
...
import de.codebarista.shopware.appbackend.sdk.util.LocaleUtils;
...
public class ShopwareController {
    ...
    ResponseEntity<Void> webHookEvent(
        ...
        app.onEvent(event, shop.getId(), LocaleUtils.getLocale(shopwareUserLanguage), shopwareLanguageId);
    ...
    ResponseEntity<String> actionEvent(
        ...
        ActionResponseDto<?> response = app.onAction(action, shop.getId(), LocaleUtils.getLocale(shopwareUserLanguage), shopwareLanguageId);
        if (response == null) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).build();
        }
